...
\only<2>{
  ...
  Все посетители разные. В этом случае будем предсказывать напрямую $y$ и границу $y_0$:
  $$\begin{array}{c}
  \mathcal{T}(X, F') = \sum_{(x,y)} \left(y_a - F'(x)\right)^2\\
  \mathcal{T}(X, F'') = \sum_{(x,y)} \left(y_b - F''(x)\right)^2\\
...
\only<3>{
  {\color{blue} Вариант 2}\\
  Предположим, что посетителей много, и нету таких, которые доставляют особенно сильно. Тогда:
  $$
  \mathcal{M}(X,F_0) = t_0-\bar{y} \sum_{(x, y)} y'(y) F_0(x)
  $$
